...
const convertToXML = (request: QLinkSerializedRequest): string => {
  ...
  return js2xml(request, options);
};

// Function to send the request
export const sendQLinkRequest = async (request: QLinkRequest): Promise<void> => {
  ...
  try {
    ...
    if (responseData.includes('<ERR_CODE>')) {
      ...
      throw new QLinkError(`QLink API error: ${errMsg || 'Unknown error'}`, Number(errCode));
    }
    ...
    if (axios.isAxiosError(error)) {
      ...
      console.error(`QLink Error: ${error.message}, Status Code: ${error.statusCode}`);
      throw error;
    } else {
